...
USB_MAGIC = b"\xA5\x5A"
# Firmware v1.2.2 is the first release described by
# ``流程图与数据帧格式_v1.2.2.pdf``.  It changed the header to carry the
# complete ``major.minor.patch`` version and moved the payload length/flags to
# offsets 14/16.  Later firmware keeps this wire format, so the comparison is
# deliberately semantic rather than a hard-coded version string.
