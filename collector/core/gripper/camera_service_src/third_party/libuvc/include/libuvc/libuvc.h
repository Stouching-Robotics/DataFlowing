...
/** Get the USB port path below the device's root hub. */
int uvc_get_port_numbers(uvc_device_t *dev, uint8_t *ports,
                         uint8_t port_count);
...
/** Pin the altsetting (and hence the ISO payload) used for this device.
 *
 * The compile-time quirks table in stream.c is a per-model default. A caller
 * that has measured which setting actually holds for a particular camera and
 * host can override it per open device handle. The override is applied to every
 * stream opened on this handle and wins over the table; pass altsetting <= 0 to
 * clear it and fall back to the table. It takes effect on the next
 * uvc_start_streaming()/uvc_stream_start(); it does not touch a running stream.
 *
 * @param[in] devh Device handle from uvc_open()
 * @param[in] altsetting bAlternateSetting to force, or <= 0 to clear
 * @param[in] payload Payload in bytes to commit and size the ISO transfers for
 * @ingroup stream
 */
uvc_error_t uvc_set_altsetting_override(
    uvc_device_handle_t *devh,
    int altsetting,
