...
/*
  The default Linux value of 100 is too deep for this service: six USB2
  isochronous cameras would submit 600 transfers (up to 32 packets each),
  exhausting the libusb/xHCI in-flight scheduling resources before all
  endpoints receive callbacks. Eight transfers per camera provide 32 ms of
  queued ISO work at the selected endpoint size and keep all six streams
  active. This remains overridable at compile time for other applications.
 */
...
struct uvc_device_handle {
  ...
  /* Per-machine altsetting override set with uvc_set_altsetting_override().
   * Zero-initialised by the calloc() in uvc_open_internal, so an untouched
   * handle falls through to the compile-time quirks table in stream.c. */
  uint8_t altsetting_override_valid;
  uint8_t altsetting_override_altsetting;
  uint16_t altsetting_override_payload;
};
