#!/bin/bash
# ============================================================================
# FS-VI-S80M → ORB-SLAM3 单目惯导 (Mono-Inertial, 仅左目 + IMU)
# 基于 run_orb_slam.sh 修改
# ============================================================================
set -e
...
ORB_CONFIG="${SCRIPT_DIR}/s80m_mono_inertial.yaml"
CAM_CONFIG="${SDK_ROOT}/config/fays_vikit_s80m.yaml"
mkdir -p "${SCRIPT_DIR}/trajectories"
TRAJ="${SCRIPT_DIR}/trajectories/traj_mono_$(date +%Y%m%d_%H%M%S).txt"

echo "========================================"
echo " FS-VI-S80M → ORB-SLAM3 (Mono-Inertial)"
echo "========================================"

# 清理旧进程 + USB 设备强制复位
pkill -f fayssense_orb_slam_mono 2>/dev/null || true
sleep 0.5
pkill -9 -f fayssense_orb_slam_mono 2>/dev/null || true
sleep 0.3
...
# 编译
BIN="${SCRIPT_DIR}/build/fayssense_orb_slam_mono"
if [ ! -f "$BIN" ]; then
...
echo "[RUN] Mono-Inertial mode. Move camera with translation+rotation for IMU init, press ENTER to set origin. Ctrl+C to exit."
