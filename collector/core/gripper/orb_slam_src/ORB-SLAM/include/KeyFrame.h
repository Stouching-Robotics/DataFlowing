...
public:
    ...
    void SetBadFlag();
    bool isBad();
    // Releases the heap payload a bad keyframe can no longer legitimately
    // serve.  Never deletes or reuses the object shell.  Returns false when the
    // keyframe is not bad yet, so a caller cannot compact a live one.
    bool CompactBadPayload();
    bool IsEraseProtected();
    bool TryReserveEraseForCleanup();
    bool HasCleanupEraseReservation();
    void CancelCleanupEraseReservation();
...
protected:
    ...
    bool mbToBeErased;
    bool mbBad;
    // Transient, deliberately not serialized.  Periodic cleanup uses this
    // to close the loop/merge protection race without publishing mbBad
    // before the covisibility and spanning-tree links have been removed.
    bool mbCleanupEraseReserved;
