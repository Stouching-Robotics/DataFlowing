...
size_t Preintegrated::MergePreviousBatch(const std::vector<Preintegrated*> &vpPrevious)
{
    if(vpPrevious.empty())
        return 0;

    for(Preintegrated* pPrevious : vpPrevious)
    {
        if(!pPrevious || pPrevious == this)
            return 0;
    }

    // Keep the same lock order as MergePrevious(): destination first, then
    // each source.  A time-window cleanup can remove a contiguous run of
    // keyframes; rebuilding the destination once avoids repeatedly copying
    // and reintegrating the ever-growing IMU history for every removed KF.
    std::unique_lock<std::mutex> destinationLock(mMutex);
    size_t measurementCount = mvMeasurements.size();
    for(Preintegrated* pPrevious : vpPrevious)
    {
        std::unique_lock<std::mutex> sourceLock(pPrevious->mMutex);
        measurementCount += pPrevious->mvMeasurements.size();
    }

    std::vector<integrable> mergedMeasurements;
    mergedMeasurements.reserve(measurementCount);
    for(Preintegrated* pPrevious : vpPrevious)
    {
        std::unique_lock<std::mutex> sourceLock(pPrevious->mMutex);
        mergedMeasurements.insert(mergedMeasurements.end(),
                                  pPrevious->mvMeasurements.begin(),
                                  pPrevious->mvMeasurements.end());
    }
    mergedMeasurements.insert(mergedMeasurements.end(),
                              mvMeasurements.begin(), mvMeasurements.end());

    const Bias updatedBias = bu;
    Initialize(updatedBias);
    mvMeasurements.reserve(mergedMeasurements.size());
    for(const integrable &measurement : mergedMeasurements)
    {
        IntegrateNewMeasurement(measurement.a, measurement.w, measurement.t);
    }
    return mergedMeasurements.size();
}

size_t Preintegrated::DiscardMeasurements()
{
    std::unique_lock<std::mutex> lock(mMutex);
    const size_t discardedCount = mvMeasurements.size();
    std::vector<integrable>().swap(mvMeasurements);
