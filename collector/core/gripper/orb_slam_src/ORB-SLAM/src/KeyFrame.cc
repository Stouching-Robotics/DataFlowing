...
namespace
{
std::atomic<unsigned long long> gCreatedKeyFrames{0};

// Retirement queue for bad KeyFrames, mirroring the MapPoint one in MapPoint.cc.
// KeyFrames cannot be deleted for the same reason MapPoints cannot -- raw
// pointers are shared across Tracking, LocalMapping and LoopClosing -- but the
// grid they carry is by far the largest per-object allocation in the map, and
// leaving it attached is what makes RSS grow without bound.  A grace period
// keeps the payload alive long enough for any pointer that was already in
// flight when the keyframe went bad to finish with it.
struct RetiredKeyFramePayload
{
    KeyFrame* identity;
    std::chrono::steady_clock::time_point retiredAt;
};

std::mutex gRetiredKeyFrameMutex;
std::deque<RetiredKeyFramePayload> gRetiredKeyFramePayloads;
// Keep identities after compaction so a repeated SetBadFlag cannot queue or
// compact the same object twice.
std::unordered_set<KeyFrame*> gKnownRetiredKeyFrames;
unsigned long long gKeyFrameCleanupPasses = 0;
unsigned long long gKeyFrameCleanupCandidates = 0;
unsigned long long gKeyFrameCleanupCompacted = 0;
std::atomic<unsigned long long> gQueuedKeyFramePayloads{0};

void QueueBadKeyFramePayload(KeyFrame* identity)
{
    std::lock_guard<std::mutex> lock(gRetiredKeyFrameMutex);
    if(!identity || !gKnownRetiredKeyFrames.insert(identity).second)
        return;
    gRetiredKeyFramePayloads.push_back(
        {identity, std::chrono::steady_clock::now()});
    gQueuedKeyFramePayloads.store(
        static_cast<unsigned long long>(gRetiredKeyFramePayloads.size()),
...
{
    return gQueuedKeyFramePayloads.load(std::memory_order_acquire);
}

BadKeyFrameCleanupStats CompactBadKeyFramePayloads(
        double minAgeSeconds, unsigned long long maxCandidates)
{
    const auto now = std::chrono::steady_clock::now();
    const auto minAge = std::chrono::duration<double>(
        std::max(0.0, minAgeSeconds));
    std::vector<RetiredKeyFramePayload> candidates;
    {
        std::lock_guard<std::mutex> lock(gRetiredKeyFrameMutex);
        ++gKeyFrameCleanupPasses;
        while(!gRetiredKeyFramePayloads.empty() &&
              now - gRetiredKeyFramePayloads.front().retiredAt >= minAge &&
              (maxCandidates == 0 || candidates.size() < maxCandidates))
        {
            candidates.push_back(gRetiredKeyFramePayloads.front());
            gRetiredKeyFramePayloads.pop_front();
        }
        gQueuedKeyFramePayloads.store(
            static_cast<unsigned long long>(gRetiredKeyFramePayloads.size()),
            std::memory_order_release);
        gKeyFrameCleanupCandidates += candidates.size();
    }

    unsigned long long compacted = 0;
    for(size_t i = 0; i < candidates.size(); ++i)
    {
        if(candidates[i].identity->CompactBadPayload())
            ++compacted;
        if((i & 31U) == 31U)
    ...
    std::lock_guard<std::mutex> lock(gRetiredKeyFrameMutex);
    gKeyFrameCleanupCompacted += compacted;
    return {
        gKeyFrameCleanupPasses,
        gKeyFrameCleanupCandidates,
        gKeyFrameCleanupCompacted,
        static_cast<unsigned long long>(gRetiredKeyFramePayloads.size())
    };
}

bool KeyFrame::CompactBadPayload()
{
    {
        // Only mbBad is read under the lock; it is never cleared once set, so
        // the swap below needs no protection.  Deliberately not held across the
        // swap: releasing a few thousand small vectors takes long enough that
        // holding mMutexConnections would stall the LocalMapping hot paths.
        unique_lock<mutex> lock(mMutexConnections);
        if(!mbBad)
            return false;
    }

    // Do not delete or reuse the object shell: ORB-SLAM3 shares raw KeyFrame*
    // across Tracking, LocalMapping and LoopClosing, and Tracking::mpLastKeyFrame
    // outlives SetBadFlag() by design.  Only release payload that a bad keyframe
    // can no longer legitimately serve.
    //
    // The feature grid is the member worth releasing: it is two
    // FRAME_GRID_COLS x FRAME_GRID_ROWS vector-of-vector grids (~150 KB for a
    // stereo keyframe), and it is read from exactly one place, GetFeaturesInArea.
    // SetBadFlag() has already removed this keyframe from the map, the
    // covisibility graph, the spanning tree, the keyframe database and every
    // MapPoint observation, so no live matching path can reach it any more.
    std::vector<std::vector<std::vector<size_t> > >().swap(mGrid);
    std::vector<std::vector<std::vector<size_t> > >().swap(mGridRight);
    return true;
}

unsigned long long GetCreatedKeyFrameCount()
{
    return gCreatedKeyFrames.load(std::memory_order_relaxed);
}
...
KeyFrame::KeyFrame():
        ...
        mbToBeErased(false), mbBad(false), mbCleanupEraseReserved(false), mHalfBaseline(0), mbCurrentPlaceRecognition(false), mnMergeCorrectedForKF(0),
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mbToBeErased(false), mbBad(false), mbCleanupEraseReserved(false), mHalfBaseline(F.mb/2), mpMap(pMap), mbCurrentPlaceRecognition(false), mNameFile(F.mNameFile), mnMergeCorrectedForKF(0),
...
{
    mnId=nNextId++;
    gCreatedKeyFrames.fetch_add(1, std::memory_order_relaxed);
...
void KeyFrame::AddConnection(KeyFrame *pKF, const int &weight)
{
    {
        unique_lock<mutex> lock(mMutexConnections);
        if(mbBad || mbCleanupEraseReserved || !pKF || pKF == this)
            return;
...
void KeyFrame::ChangeParent(KeyFrame *pKF)
{
    if(!pKF)
    {
        cout << "ERROR: Change parent KF, the parent is null" << endl;
        return;
    }
    unique_lock<mutex> lockCon(mMutexConnections);
    if(pKF == this)
...
{
    unique_lock<mutex> lockCon(mMutexConnections);
    if(mbBad || mbCleanupEraseReserved || !pKF)
        return;
    mbNotErase = true;
...
{
    unique_lock<mutex> lock(mMutexConnections);
    if(!mbBad && !mbCleanupEraseReserved)
        mbNotErase = true;
}
...
{
    vector<KeyFrame*> connectedKeyFrames;
    {
        unique_lock<mutex> lock(mMutexConnections);
        if(!mpMap || mnId==mpMap->GetInitKFid())
        {
            mbCleanupEraseReserved = false;
            return;
        }
        else if(mbBad)
        {
        ...
        connectedKeyFrames.reserve(mConnectedKeyFrameWeights.size());
        for(const pair<KeyFrame* const,int> &connection : mConnectedKeyFrameWeights)
        {
            if(connection.first && connection.first != this)
                connectedKeyFrames.push_back(connection.first);
        }
    }

    // Iterate over a locked snapshot.  The former unlocked map traversal was
    // unsafe while loop-closing or another KF updated reciprocal links.
    for(KeyFrame* connectedKeyFrame : connectedKeyFrames)
    {
        connectedKeyFrame->EraseConnection(this);
    }
...

    {
        ...
        if(mpParent && mpParent != this && !mpParent->isBad())
        ...
        while(!mspChildrens.empty())
        {
            ...
            for(set<KeyFrame*>::iterator sit=mspChildrens.begin(), send=mspChildrens.end(); sit!=send; sit++)
            {
                ...
                {
                    KeyFrame* connectedKeyFrame = vpConnected[i];
                    if(!connectedKeyFrame || connectedKeyFrame == this || connectedKeyFrame->isBad())
                        continue;
                    for(set<KeyFrame*>::iterator spcit=sParentCandidates.begin(), spcend=sParentCandidates.end(); spcit!=spcend; spcit++)
                    {
                        KeyFrame* parentCandidate = *spcit;
                        if(parentCandidate && connectedKeyFrame == parentCandidate)
                        {
                            int w = pKF->GetWeight(connectedKeyFrame);
                            if(w>max)
                            {
                                pC = pKF;
                                pP = connectedKeyFrame;
                                max = w;
                                bContinue = true;
                            }
                        }
        ...
        KeyFrame* fallbackParent = mpParent;
        size_t parentDepth = 0;
        while(fallbackParent && fallbackParent != this &&
              fallbackParent->isBad() && parentDepth++ < 1024)
        {
            fallbackParent = fallbackParent->GetParent();
        }
        if(fallbackParent == this ||
           (fallbackParent && fallbackParent->isBad()))
        {
            fallbackParent = NULL;
        }

        if(!mspChildrens.empty() && fallbackParent)
        {
            for(set<KeyFrame*>::iterator sit=mspChildrens.begin(); sit!=mspChildrens.end(); sit++)
            {
                KeyFrame* child = *sit;
                if(child && child != this && !child->isBad())
                    child->ChangeParent(fallbackParent);
            }
        }
        mspChildrens.clear();

        if(fallbackParent){
            fallbackParent->EraseChild(this);
            mTcp = mTcw * fallbackParent->GetPoseInverse();
            mpParent = fallbackParent;
        }
        mbBad = true;
        mbCleanupEraseReserved = false;
    }
    ...
    // Fully unlinked: no live path can match against this keyframe any more, so
    // its grid is dead weight.  Queued rather than freed here so that a pointer
    // already in flight when it went bad still sees valid payload.
    QueueBadKeyFramePayload(this);
}
...
bool KeyFrame::IsEraseProtected()
{
    unique_lock<mutex> lock(mMutexConnections);
    return mbNotErase || mbToBeErased || mbBad || mbCleanupEraseReserved;
}

bool KeyFrame::TryReserveEraseForCleanup()
{
    unique_lock<mutex> lock(mMutexConnections);
    if(!mpMap || mnId == mpMap->GetInitKFid() || mbNotErase ||
       mbToBeErased || mbBad || mbCleanupEraseReserved)
        return false;

    // Reserve this identity before the inertial chain is changed.  Do not
    // publish mbBad until SetBadFlag() has removed the visual graph links.
    mbCleanupEraseReserved = true;
    return true;
}

bool KeyFrame::HasCleanupEraseReservation()
{
    unique_lock<mutex> lock(mMutexConnections);
    return mbCleanupEraseReserved && !mbBad;
}

void KeyFrame::CancelCleanupEraseReservation()
{
    unique_lock<mutex> lock(mMutexConnections);
    if(!mbBad)
        mbCleanupEraseReserved = false;
...
vector<size_t> KeyFrame::GetFeaturesInArea(const float &x, const float &y, const float &r, const bool bRight) const
{
    ...
    // A keyframe whose payload was reclaimed (CompactBadPayload) has an empty
    // grid while mnGridCols/mnGridRows stay at their compile-time values, so the
    // cell loops below would index a cleared vector.  Answer with nothing
    // instead: a bad keyframe legitimately has no features to match against, and
    // a stale reader gets a benign empty result rather than an out-of-bounds
    // read.
    if(bRight ? mGridRight.empty() : mGrid.empty())
        return vIndices;
