...
{
    if(!BindFaysBackgroundThread("fays-loop-close", "loop closing"))
    {
        SetFinish();
        return;
    }
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(bFindedRegion)
            {
                if(mbMergeDetected)
                {
                    ...
                    // Fix 8: Cap PR timing vectors to prevent unbounded growth (1000 samples max)
                    if (vdPR_CurrentTime.size() > 1000) {
                        vdPR_CurrentTime.erase(vdPR_CurrentTime.begin());
                        vdPR_MatchedTime.erase(vdPR_MatchedTime.begin());
                        vnPR_TypeRecogn.erase(vnPR_TypeRecogn.begin());
                    }
