...
Map::~Map()
{
    // Fix 3: Properly delete MapPoints and KeyFrames instead of just clearing pointers.
    // Original code only called clear() on the sets, leaking all heap objects.
    // We iterate in reverse dependency order: MapPoints first (they reference KFs),
    // then KeyFrames (they may reference each other via covisibility graph).
