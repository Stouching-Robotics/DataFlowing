...
namespace
{
struct RetiredMapPointPayload
{
    MapPoint* identity;
    std::chrono::steady_clock::time_point retiredAt;
};

std::mutex gRetiredPayloadMutex;
std::deque<RetiredMapPointPayload> gRetiredPayloads;
// Keep identities after compaction so repeated SetBadFlag calls cannot queue
// or compact the same object twice.
std::unordered_set<MapPoint*> gKnownRetiredMapPoints;
unsigned long long gCleanupPasses = 0;
unsigned long long gCleanupCandidates = 0;
unsigned long long gCleanupCompacted = 0;
std::atomic<unsigned long long> gCreatedMapPoints{0};
std::atomic<unsigned long long> gQueuedMapPointPayloads{0};

void QueueBadMapPointPayload(MapPoint* identity)
{
    std::lock_guard<std::mutex> lock(gRetiredPayloadMutex);
    if(!identity || !gKnownRetiredMapPoints.insert(identity).second)
        return;
    gRetiredPayloads.push_back(
        {identity, std::chrono::steady_clock::now()});
    gQueuedMapPointPayloads.store(
        static_cast<unsigned long long>(gRetiredPayloads.size()),
...
{
    return gCreatedMapPoints.load(std::memory_order_relaxed);
}

unsigned long long GetQueuedBadMapPointPayloadCount()
{
    return gQueuedMapPointPayloads.load(std::memory_order_acquire);
}
        ...
        double minAgeSeconds, unsigned long long maxCandidates)
{
    ...
    std::vector<RetiredMapPointPayload> candidates;
    {
        std::lock_guard<std::mutex> lock(gRetiredPayloadMutex);
        ++gCleanupPasses;
        while(!gRetiredPayloads.empty() &&
              now - gRetiredPayloads.front().retiredAt >= minAge &&
              (maxCandidates == 0 || candidates.size() < maxCandidates))
        {
            candidates.push_back(gRetiredPayloads.front());
            gRetiredPayloads.pop_front();
        }
        gQueuedMapPointPayloads.store(
            static_cast<unsigned long long>(gRetiredPayloads.size()),
            std::memory_order_release);
        gCleanupCandidates += candidates.size();
    }
    ...
    mnOriginMapId(pMap->GetId())
{
    ...
    mnId=nNextId++;
    gCreatedMapPoints.fetch_add(1, std::memory_order_relaxed);
}
...
bool MapPoint::CompactBadPayload()
{
    ...
    // Do not delete or reuse the object shell: ORB-SLAM3 shares raw pointers
    // across Tracking, LocalMapping and LoopClosing.  Only release detached
    // heap payload that a bad point cannot use again.
    mDescriptor.release();
    map<long unsigned int, int>().swap(mBackupObservationsId1);
    map<long unsigned int, int>().swap(mBackupObservationsId2);
    return true;
}
