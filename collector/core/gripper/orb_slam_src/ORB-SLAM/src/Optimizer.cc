...
void Optimizer::FullInertialBA(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId, bool *pbStopFlag, bool bInit, float priorG, float priorA, Eigen::VectorXd *vSingVal, bool *bHess)
{
    ...
    // 因「观测到的关键帧不在顶点集里」而被跳过的观测数（见下方守卫）。
    // 换图之后不为 0 属正常，长期为 0 才是常态。
    unsigned long long nAbsentKFVertex = 0;
    ...
    for(size_t i=0; i<vpMPs.size(); i++)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                // 观测到的关键帧必须已在上面的关键帧循环里登记成顶点（id = mnId）。
                // Atlas 换图后 MapPoint 仍可能持有旧图关键帧的观测，而旧图关键帧不在
                // GetAllKeyFrames() 里，vertex() 就返回 NULL。原代码在单目/双目/右目
                // 三处都是拿到 VP 立刻 VP->fixed()，实测在 0x54（g2o
                // OptimizableGraph::Vertex::_fixed 的偏移）处以 NULL 解引用崩溃
                // （2026-09-11 11:14，fays-local-map 线程）。这里在触碰任何观测数据
                // 之前统一判空并跳过该观测：该地图点少一条约束，但不会把 NULL 带进图。
    ...
    if(nAbsentKFVertex > 0)
    {
        // 一次调用只报一行汇总（不是逐观测刷屏）。文案不得命中
        // core/gripper/slam/protocol.py 的 _ERROR_RE，否则会被当 error 弹到 UI。
        static unsigned long long nAbsentKFVertexTotal = 0;
        nAbsentKFVertexTotal += nAbsentKFVertex;
        std::cerr << "[FULL_BA_RECOVERY] optimizer=FullInertialBA"
                  << " skipped_obs=" << nAbsentKFVertex
                  << " max_kf_id=" << maxKFid
                  << " b_init=" << (bInit ? 1 : 0)
                  << " total=" << nAbsentKFVertexTotal << "\n";
    }
...
{
    // A visual-only frame need not carry an inertial prior. Never create an
    // inertial graph from missing state or fabricate a zero-information prior.
    if (!pFrame->mpLastKeyFrame || !pFrame->mpImuPreintegrated || !(pFrame->mpImuPreintegrated->dT > 0.f))
    {
        static std::atomic<unsigned long long> fallbackCount{0};
        const auto count = ++fallbackCount;
        if (count <= 5 || count % 500 == 0)
            std::cerr << "[IMU_RECOVERY] optimizer=PoseInertialOptimizationLastKeyFrame"
                      << " frame=" << pFrame->mnId
                      << " timestamp=" << pFrame->mTimeStamp
                      << " previous_prior=" << (pFrame->mpPrevFrame && pFrame->mpPrevFrame->mpcpi ? 1 : 0)
                      << " fallback=visual count=" << count << std::endl;
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
{
    // A visual-only frame need not carry an inertial prior. Never create an
    // inertial graph from missing state or fabricate a zero-information prior.
    if (!pFrame->mpPrevFrame || !pFrame->mpPrevFrame->mpcpi || !pFrame->mpImuPreintegratedFrame || !pFrame->mpImuPreintegrated || !(pFrame->mpImuPreintegratedFrame->dT > 0.f) || !(pFrame->mpImuPreintegrated->dT > 0.f))
    {
        static std::atomic<unsigned long long> fallbackCount{0};
        const auto count = ++fallbackCount;
        if (count <= 5 || count % 500 == 0)
            std::cerr << "[IMU_RECOVERY] optimizer=PoseInertialOptimizationLastFrame"
                      << " frame=" << pFrame->mnId
                      << " timestamp=" << pFrame->mTimeStamp
                      << " previous_prior=" << (pFrame->mpPrevFrame && pFrame->mpPrevFrame->mpcpi ? 1 : 0)
                      << " fallback=visual count=" << count << std::endl;
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections)
{
    ...
    auto optimizerVertex = [&optimizer, nMaxKFid](const long unsigned int id)
        -> g2o::OptimizableGraph::Vertex*
    {
        if(id > nMaxKFid)
            return static_cast<g2o::OptimizableGraph::Vertex*>(NULL);
        return dynamic_cast<g2o::OptimizableGraph::Vertex*>(
            optimizer.vertex(static_cast<int>(id)));
    };
    unsigned int skippedInvalidLoopEdges = 0;
    unsigned int skippedInvalidInertialEdges = 0;
    unsigned int skippedInvalidGraphEdges = 0;

    if(!pLoopKF || !pCurKF || pLoopKF->isBad() || pCurKF->isBad() ||
       !optimizerVertex(pLoopKF->mnId) || !optimizerVertex(pCurKF->mnId))
    {
        cerr << "[LOOP_GUARD] abort 4DoF optimization: missing current/loop vertex"
             << endl;
        return;
    }
    set<pair<long unsigned int,long unsigned int> > sInsertedEdges;
    ...
    {
        KeyFrame* pKF = mit->first;
        if(!pKF || pKF->isBad() || !optimizerVertex(pKF->mnId))
        {
            ++skippedInvalidLoopEdges;
            continue;
        }
        const long unsigned int nIDi = pKF->mnId;
        const set<KeyFrame*> &spConnections = mit->second;
        ...
        for(set<KeyFrame*>::const_iterator sit=spConnections.begin(), send=spConnections.end(); sit!=send; sit++)
        {
            KeyFrame* connectedKeyFrame = *sit;
            if(!connectedKeyFrame || connectedKeyFrame->isBad() ||
               !optimizerVertex(connectedKeyFrame->mnId))
            {
                ++skippedInvalidLoopEdges;
                continue;
            }
            const long unsigned int nIDj = connectedKeyFrame->mnId;
            if((nIDi!=pCurKF->mnId || nIDj!=pLoopKF->mnId) && pKF->GetWeight(connectedKeyFrame)<minFeat)
                continue;
            ...
            e->setVertex(1, optimizerVertex(nIDj));
            e->setVertex(0, optimizerVertex(nIDi));
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        if(prevKF)
        {
            ...
            if(prevKF->isBad() || prevKF->GetMap() != pMap ||
               !optimizerVertex(nIDj))
            {
                ++skippedInvalidInertialEdges;
                cerr << "[LOOP_GUARD] skip invalid inertial edge current="
                     << nIDi << " prev=" << nIDj << endl;
                prevKF = static_cast<KeyFrame*>(NULL);
            }
        }
        ...
        {
            KeyFrame* pLKF = *sit;
            if(pLKF && !pLKF->isBad() && optimizerVertex(pLKF->mnId) &&
               pLKF->mnId<pKF->mnId)
            {
                g2o::Sim3 Swl;
                ...
                e->setVertex(0, optimizerVertex(nIDi));
                e->setVertex(1, optimizerVertex(pLKF->mnId));
                e->information() = matLambda;
                optimizer.addEdge(e);
            }
            else if(pLKF)
        ...
        for(vector<KeyFrame*>::const_iterator vit=vpConnectedKFs.begin(); vit!=vpConnectedKFs.end(); vit++)
        {
            ...
            if(pKFn && pKFn!=pParentKF && pKFn!=prevKF && pKFn!=pKF->mNextKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn))
            {
                if(!pKFn->isBad() && pKFn->mnId<pKF->mnId)
                {
                    ...
                    e->setVertex(0, optimizerVertex(nIDi));
                    e->setVertex(1, optimizerVertex(pKFn->mnId));
                    e->information() = matLambda;
                    optimizer.addEdge(e);
                }
            }
    ...
    if(skippedInvalidLoopEdges || skippedInvalidInertialEdges ||
       skippedInvalidGraphEdges)
    {
        cerr << "[LOOP_GUARD] skipped edges loop=" << skippedInvalidLoopEdges
             << " inertial=" << skippedInvalidInertialEdges
             << " graph=" << skippedInvalidGraphEdges << endl;
    }

    if(optimizer.edges().empty())
    {
        cerr << "[LOOP_GUARD] abort 4DoF optimization: no valid edges"
             << endl;
        return;
    }
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(!Vi)
        {
            cerr << "[LOOP_GUARD] skip pose recovery for missing KF vertex="
                 << nIDi << endl;
            continue;
        }
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        KeyFrame* pRefKF = pMP->GetReferenceKeyFrame();
        if(!pRefKF || pRefKF->isBad() || !optimizerVertex(pRefKF->mnId))
            continue;
        nIDr = pRefKF->mnId;
