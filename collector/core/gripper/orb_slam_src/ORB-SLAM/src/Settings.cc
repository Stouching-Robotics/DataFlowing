...
namespace ORB_SLAM3 {
    ...
    void Settings::readCamera2(cv::FileStorage &fSettings) {
        ...
        // For Rectified cameras, Camera2 has same intrinsics as Camera1
        if(cameraType_ == Rectified){
            vector<float> vCalib2 = {
                calibration1_->getParameter(0),
                calibration1_->getParameter(1),
                calibration1_->getParameter(2),
                calibration1_->getParameter(3)
            };
            calibration2_ = new Pinhole(vCalib2);
            originalCalib2_ = new Pinhole(vCalib2);
        }
