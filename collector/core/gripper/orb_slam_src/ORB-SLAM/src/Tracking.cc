...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq):
    ...
    mnLastRelocFrameId(0), time_recently_lost(5.0), mbImuPredictionValid(false),
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    const auto ksqGrabStart = std::chrono::steady_clock::now();
    const auto ksqGrayStart = ksqGrabStart;
    ...
    const auto ksqGrayEnd = std::chrono::steady_clock::now();
    mKsqGrabGrayMs = std::chrono::duration<double, std::milli>(
        ksqGrayEnd - ksqGrayStart).count();

    //cout << "Incoming frame creation" << endl;

    const auto ksqFrameStart = std::chrono::steady_clock::now();
    ...
    const auto ksqFrameEnd = std::chrono::steady_clock::now();
    mKsqGrabFrameMs = std::chrono::duration<double, std::milli>(
        ksqFrameEnd - ksqFrameStart).count();
    ...
    const auto ksqTrackStart = std::chrono::steady_clock::now();
    Track();
    const auto ksqTrackEnd = std::chrono::steady_clock::now();
    mKsqGrabTrackMs = std::chrono::duration<double, std::milli>(
        ksqTrackEnd - ksqTrackStart).count();
    //cout << "Tracking end" << endl;
    const auto ksqPostStart = std::chrono::steady_clock::now();
    UpdateRuntimeTrackingLocalCounts(
        static_cast<unsigned long long>(mvpLocalKeyFrames.size()),
        static_cast<unsigned long long>(mvpLocalMapPoints.size()));
    const auto ksqPostEnd = std::chrono::steady_clock::now();
    mKsqGrabPostMs = std::chrono::duration<double, std::milli>(
        ksqPostEnd - ksqPostStart).count();
    mKsqGrabTotalMs = std::chrono::duration<double, std::milli>(
        ksqPostEnd - ksqGrabStart).count();
...
bool Tracking::PreintegrateIMU()
{
    ...
    mbImuPredictionValid = false;
    if(!mCurrentFrame.mpPrevFrame)
    {
        Verbose::PrintMess("non prev frame ", Verbose::VERBOSITY_NORMAL);
        // Every skip path in this function must still write the chain head:
        // the image constructors leave mpLastKeyFrame indeterminate and
        // UpdateLocalKeyFrames() may walk the temporal chain from it.
    ...
    size_t queuedImu = 0;
    double queuedFirstTs = -1., queuedLastTs = -1.;
    {
        unique_lock<mutex> lock(mMutexImuQueue);
        queuedImu = mlQueueImuData.size();
        if (queuedImu) {
            queuedFirstTs = mlQueueImuData.front().t;
            queuedLastTs = mlQueueImuData.back().t;
        }
    }
    mvImuFromLastFrame.reserve(queuedImu);
    if(queuedImu == 0)
    {
        static unsigned long long empty_warn_count = 0;
        ++empty_warn_count;
        if(empty_warn_count <= 5U || empty_warn_count % 500U == 0U)
        {
            cout << "Not IMU data in mlQueueImuData!!"
                 << " frame=" << mCurrentFrame.mnId
                 << " previous_ts=" << mCurrentFrame.mpPrevFrame->mTimeStamp
                 << " current_ts=" << mCurrentFrame.mTimeStamp
                 << " warned=" << empty_warn_count
                 << " -> skip IMU prediction" << endl;
        }
        mCurrentFrame.mpLastKeyFrame = mpLastKeyFrame;
        mCurrentFrame.setIntegrated();
        return false;
    ...
    if(n <= 0){
        static unsigned long long insufficient_warn_count = 0;
        ++insufficient_warn_count;
        if(insufficient_warn_count <= 5U ||
           insufficient_warn_count % 500U == 0U)
        {
            cout << "Insufficient IMU measurements"
                 << " samples=" << mvImuFromLastFrame.size()
                 << " queued_before=" << queuedImu
                 << " queued_first_ts=" << queuedFirstTs
                 << " queued_last_ts=" << queuedLastTs
                 << " coverage_s=" << queuedLastTs-mCurrentFrame.mTimeStamp
                 << " frame=" << mCurrentFrame.mnId
                 << " previous_ts=" << mCurrentFrame.mpPrevFrame->mTimeStamp
                 << " current_ts=" << mCurrentFrame.mTimeStamp
                 << " first_ts=" << (mvImuFromLastFrame.empty() ? -1. : mvImuFromLastFrame.front().t)
                 << " last_ts=" << (mvImuFromLastFrame.empty() ? -1. : mvImuFromLastFrame.back().t)
                 << " warned=" << insufficient_warn_count
                 << " -> skip IMU prediction" << endl;
        }
        // The image constructors leave mpLastKeyFrame indeterminate, and this
        // frame was built by copy-assigning a temporary (GrabImageStereo), so
        // skipping the assignment at the end of this function would let that
        // garbage reach UpdateLocalKeyFrames()'s temporal-chain walk. A frame
        // without inertial payload still knows which keyframe it follows.
        mCurrentFrame.mpLastKeyFrame = mpLastKeyFrame;
        mCurrentFrame.setIntegrated();
        return false;
    }
    ...
    {
        std::cerr << "[IMU_RECOVERY] missing keyframe accumulator frame="
                  << mCurrentFrame.mnId << " fallback=visual" << std::endl;
        // Same reason as the guard above: never leave the chain head at the
        // constructor's indeterminate value.
        mCurrentFrame.mpLastKeyFrame = mpLastKeyFrame;
        mCurrentFrame.setIntegrated();
        return false;
    }
    ...
    for(int i=0; i<n; i++)
    {
        ...
        if (!mpImuPreintegratedFromLastKF)
        {
            static int _null_warn_count = 0;
            if (++_null_warn_count <= 5)
                cout << "mpImuPreintegratedFromLastKF does not exist (skipping IMU integration)" << endl;
            continue;  // Fix 10: skip this IMU measurement instead of crashing on null dereference
        }
...
{
    // Recently-lost tracking also calls this method directly.
    if (!mbImuPredictionValid)
        return false;
    if(!mCurrentFrame.mpPrevFrame)
    {
    ...
    if(mbMapUpdated)
    {
        if(!mpLastKeyFrame || !mpImuPreintegratedFromLastKF)
        {
            static unsigned long long kf_payload_warn_count = 0;
            ++kf_payload_warn_count;
            if(kf_payload_warn_count <= 5U ||
               kf_payload_warn_count % 500U == 0U)
            {
                cout << "Invalid keyframe IMU preintegration payload"
                     << " key_frame=" << (mpLastKeyFrame ? "yes" : "no")
                     << " payload=" << (mpImuPreintegratedFromLastKF ? "yes" : "no")
                     << " warned=" << kf_payload_warn_count
                     << " -> skip IMU prediction" << endl;
            }
            return false;
        }
    ...
    else if(!mbMapUpdated)
    {
        ...
        {
            static unsigned long long frame_payload_warn_count = 0;
            ++frame_payload_warn_count;
            if(frame_payload_warn_count <= 5U ||
               frame_payload_warn_count % 500U == 0U)
            {
                cout << "Missing frame IMU preintegration payload"
                     << " warned=" << frame_payload_warn_count
                     << " -> skip IMU prediction" << endl;
            }
            return false;
        }
...
void Tracking::Track()
{
    // Historical Fays baseline timestamp used only by tracking diagnostics.
    static double t_origin = -1.0;
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            if(bOK)
            {
                // Preserve the IMU/motion-model prediction before local-map
                // tracking.  This matches the historical Fays recorder core:
                // reject only very large visual jumps and fall back to the
                // inertial prediction for that frame.
                const Sophus::SE3f T_before = mCurrentFrame.GetPose();
                bOK = TrackLocalMap();

                // Temporarily disabled 2026-09-01.  The fixed thresholds
                // rejected normal motion and caused accumulated IMU-only
                // offsets.  Keep TrackLocalMap's visual-inertial result.
                if (false &&
                    (mSensor == System::IMU_MONOCULAR ||
                    mSensor == System::IMU_STEREO ||
                    mSensor == System::IMU_RGBD))
                {
                    const Sophus::SE3f T_after = mCurrentFrame.GetPose();
                    const Sophus::SE3f T_last = mLastFrame.GetPose();
                    const Sophus::SE3f T_rel = T_last.inverse() * T_after;
                    const float trans_dist = T_rel.translation().norm();
                    const Eigen::Matrix3f R_rel = T_rel.rotationMatrix();
                    const float trace = R_rel.trace();
                    const float cos_theta = std::max(
                        -1.0f, std::min(1.0f, (trace - 1.0f) / 2.0f));
                    const float rot_angle =
                        std::acos(cos_theta) * 180.0f / M_PI;

                    if ((trans_dist > 0.05f || rot_angle > 1.19f) &&
                        mpAtlas->isImuInitialized())
                    {
                        ...
                        cout << "[IMU-FILL] visual discarded  t="
                             << (mCurrentFrame.mTimeStamp - t_origin)
                             << "  trans=" << trans_dist * 1000.0f
                             << "mm  rot=" << rot_angle
                             << "deg  -> using IMU prediction" << endl;
                    }
                }
        ...
        // ResetFrameIMU uses mCurrentFrame/mLastFrame directly.  Do not
        // allocate and immediately destroy unused frame/preintegration copies
        // here: an IMU dropout leaves mpImuPreintegratedFrame null, and the
        // pointer-copy constructor dereferences it unconditionally.
        ...
        // Fix 2: Free previous frame's IMU preintegration before overwriting mLastFrame.
        // After PreintegrateIMU(), mCurrentFrame has a new Preintegrated object while
        // mLastFrame still holds the old one. The Frame copy constructor does a shallow
        // pointer copy, so the old Preintegrated would be leaked. Original ORB-SLAM3
        // never freed these — in batch mode the process exits, but in live mode they
        // accumulate and cause OOM.
        if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
        {
            if (mLastFrame.mpImuPreintegratedFrame &&
                mLastFrame.mpImuPreintegratedFrame != mCurrentFrame.mpImuPreintegratedFrame)
            {
                delete mLastFrame.mpImuPreintegratedFrame;
                mLastFrame.mpImuPreintegratedFrame = nullptr;
            }
        }
    ...
    if(mState==OK || mState==RECENTLY_LOST)
    {
        ...
        // Fix 1: Cap history to prevent unbounded memory growth during long sessions.
        // Keep last 3000 entries (~100 seconds at 30fps), enough for IMU reset recovery.
        // Original code never trimmed these lists — they grew forever (OOM after hours).
        while (mlRelativeFramePoses.size() > 3000)
        {
            mlRelativeFramePoses.pop_front();
            mlpReferences.pop_front();
            mlFrameTimes.pop_front();
            mlbLost.pop_front();
        }
    }
    ...
    if (t_origin < 0.0 && mState == OK)
        t_origin = mCurrentFrame.mTimeStamp;

#ifdef REGISTER_TIMES
    // Fix 7: Cap performance timing vectors to prevent unbounded growth.
    // In batch mode these are only dumped at shutdown; in live mode they grow forever.
    const size_t MAX_TIMING_SAMPLES = 20000;
    if (vdORBExtract_ms.size() > MAX_TIMING_SAMPLES) {
        vdORBExtract_ms.clear();   vdStereoMatch_ms.clear();
        vdIMUInteg_ms.clear();     vdPosePred_ms.clear();
        vdLMTrack_ms.clear();      vdNewKF_ms.clear();
    }
#endif
}
...
bool Tracking::TrackLocalMap()
{
    ...
    int inliers;
    if (!mpAtlas->isImuInitialized() || !mbImuPredictionValid)
        Optimizer::PoseOptimization(&mCurrentFrame);
    else
    {
        if(mCurrentFrame.mnId<=mnLastRelocFrameId+mnFramesToResetIMU)
...
bool Tracking::Relocalization()
{
    ...
    if(!bMatch)
    {
        // Fix 6: Free MLPnPsolver objects allocated during relocalization.
        for (size_t i = 0; i < vpMLPnPsolvers.size(); i++)
            delete vpMLPnPsolvers[i];
        return false;
    }
    else
    {
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame* pCurrentKeyFrame)
{
    ...
    // A rejected IMU frame still completes preprocessing, but has no
    // inertial payload. Preserve its visual estimate rather than dereference
    // missing keyframe/preintegration state during asynchronous map updates.
