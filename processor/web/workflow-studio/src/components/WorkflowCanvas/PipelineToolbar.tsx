...
export function PipelineToolbar({ onOpenDrawer }: Props) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 bg-gray-900 border-b border-gray-800 shrink-0">
      <button onClick={onOpenDrawer} className="text-gray-400 hover:text-white p-1.5 rounded hover:bg-gray-800" title={t('toolbar.workflowList')}>☰</button>
      ...
      {showNameInput ? (
        <>
          ...
          <button onClick={confirmSaveAs} className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-2 py-1 rounded">{t('toolbar.save')}</button>
        </>
      ) : (
      ...
      <button onClick={newWorkflow} className="text-xs text-gray-400 hover:text-white px-2 py-1.5 rounded hover:bg-gray-800">{t('toolbar.new')}</button>
      <button onClick={handleSave} disabled={isSaving} className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded disabled:opacity-50">{isSaving ? '...' : t('toolbar.save')}</button>
      <button onClick={handleSaveAs} className="text-xs text-gray-400 hover:text-white px-2 py-1.5 rounded hover:bg-gray-800">{t('toolbar.saveAs')}</button>
      <button onClick={handleExport} className="text-xs text-gray-400 hover:text-white px-2 py-1.5 rounded hover:bg-gray-800">{t('toolbar.export')}</button>
      <button onClick={handleRun} className="text-xs bg-green-700 hover:bg-green-600 text-white px-3 py-1.5 rounded">▶ {t('toolbar.run')}</button>
    </div>
  );
}
